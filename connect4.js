class Game {
  ...
  placeInTable(y, x) {
    ...
    piece.style.backgroundColor = this.current.color;
  ...
  handleClick(evt) {
    ...
    // check for win
    if (this.checkForWin()) {
      return this.endGame(`Player ${this.current.color} won!`);
    }
    ...
    // switch players
    this.current = this.current === this.players[0] ? this.players[1] : this.players[0];
  }
